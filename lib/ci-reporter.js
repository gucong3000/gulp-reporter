...
const appveyorApiUrl = process.env.APPVEYOR_API_URL && new URL('api/build/compilationmessages', appveyorApiUrl).href;
const category = {
	warn: 'Warning',
	info: 'Information',
	error: 'Error',
...
function getReportPath() {
	return path.join(getEnv('TEST_REPORTS') || getEnv('REPORT_PATH') || 'test-reports', 'lint-junit-' + (getEnv('BUILD_ID') || getEnv('BUILD_NUM') || Date.now()) + '.xml');
}
...
function junit(errors) {
	...
	return fs.outputFile(getReportPath(), output);
}
