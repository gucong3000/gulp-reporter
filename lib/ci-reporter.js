...
const reportPath = path.join(getEnv('TEST_REPORTS') || getEnv('REPORTS') || getEnv('REPORT_PATH') || 'test-reports', 'lint-junit-' + (getEnv('BUILD_ID') || getEnv('BUILD_NUM') || getEnv('BUILD_NUMBER') || Date.now()) + '.xml');
const category = {
	warn: 'Warning',
	info: 'Information',
	error: 'Error',
...
function getEnv(name) {
	return (privatePrefix && process.env[privatePrefix + name]) || process.env['CI_' + name] || process.env[name];
}
