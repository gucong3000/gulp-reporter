...
function highlight(code, extname) {
	if (gutil.colors.enabled) {
		if (extname) {
			try {
				return emphasize.highlight(extname.slice(1), code).value;
			} catch (ex) {
				//
			}
		}
...
class LintError extends Error {
	...
	toString() {
		...
		if (error.source) {
			addMsg([
				addLineNumbers(highlight(error.source, path.extname(error.fileName)), error.lineNumber)
			]);
		}
