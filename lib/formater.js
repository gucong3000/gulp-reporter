...
// 		`\\u{${ Number(c.codePointAt(0)).toString(16) }}`
// 	)).join('');
// }
// Object.keys(icon).forEach(key => {
// 	console.log(`${key}: '${ encode(icon[key]) }'`);
// });
...
function formatError(error, fmtOpts) {
	...
	if (error.demote || !severity || !icon[severity]) {
		severity = 'success';
	}
	...
	if (process.platform === 'win32' && !process.env.ConEmuPID) {
		message[0] = message[0].replace('\u{fe0f}', process.env.VSCODE_PID ? ' ' : '');
	}
