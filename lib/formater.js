...
function justify(left, right, width, char) {
	if (left) {
		width -= stringWidth(gutil.colors.stripColor(left));
	} else {
		left = '';
	}
	if (right) {
		width -= stringWidth(gutil.colors.stripColor(right));
	} else {
		right = '';
	}
	if (!char) {
...
function formatError(error, fmtOpts) {
	...
	if (subMsg.length) {
		...
		const mainMessageWidth = error.message.length + fmtOpts.lineNumberWidth + fmtOpts.columnNumberWidth + subMsg.length + 8;
		if (mainMessageWidth > fmtOpts.termColumns) {
