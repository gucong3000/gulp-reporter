...
function formatError(error, cache) {
	const message = [];
	...
	if (error.source) {
		addMsg([
			cache[error.source] || (cache[error.source] = addLineNumbers(highlight(error), error.lineNumber))
		]);
	}
...
function formater(file) {
	const cache = {};
	const errors = file.report.errors.map(error => formatError(error, cache).replace(/^/gm, '\t'));
