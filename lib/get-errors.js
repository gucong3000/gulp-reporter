'use strict';
const EditorConfigError = require('./editorconfig-error');
const PostCSSError = require('./postcss-error');
const CssLintError = require('./csslint-error');
...
function getErrors (file, options) {
	...
	if (file.editorconfig) {
		file.editorconfig.errors.forEach(error => errors.push(new EditorConfigError(error)));
	}
	if (file.postcss) {
		file.postcss.messages.forEach(message => errors.push(new PostCSSError(message)));
	}
	if (file.csslint) {
		file.csslint.report.messages.forEach(message => errors.push(new CssLintError(message)));
	}
	if (file.eslint) {
