...
describe('JSHint', () => {
	it('JSHintError', () => {
		...
		assert.strictEqual(error.message, 'tesecase');
		const error2 = new JSHintError({
			raw: "Unreachable '{a}' after '{b}'.",
			message: 'tesecase',
			file: __filename,
		});
		assert.notStrictEqual(error2.message, 'tesecase');
	});
	...
	it('console reporter', () => {
		...
		return sandbox.gotError(stream).then(error => {
			...
			assert.strictEqual(error.message, 'Lint failed for: test/fixtures/jshint/invalid.js');
